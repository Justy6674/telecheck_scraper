...
/**
 * NORTHERN TERRITORY DISASTER SCRAPER FOR RAILWAY DEPLOYMESA
 * Scrapes SA disasters and updates database
...
console.log('=
 SA Disaster Scraper Service Starting...');
...
app.get('/', (req, res) => {
  res.json({ 
    status: 'running',
    state: 'SA',
    service: 'South Australia Disaster Scraper',
...
app.get('/scrape', async (req, res) => {
  res.json({ message: 'SA scraping started' });
  scrapeSA().catch(console.error);
});
...
async function scrapeSA() {
  console.log(`\n=
 Scraping SA disasters at ${new Date().toISOString()}`);
  ...
  try {
    await page.goto('https://www.disasterassist.gov.au/find-a-disaster/australian-disasters?state=sa', {
  ...
  console.log(` SA scraping complete: ${allDisasters.length} disasters, ${activeCount} active, ${uniqueLGAs.length} LGAs affected`);
}
