...
/**
 * NORTHERN TERRITORY DISASTER SCRAPER FOR RAILWAY DEPLOYMENT
 * Scrapes NT disasters and updates database
 * Expected: 2 pages, ~40 disasters
 */
...
console.log('=
 NT Disaster Scraper Service Starting...');
...
app.get('/', (req, res) => {
  res.json({ 
    status: 'running',
    state: 'NT',
    service: 'Northern Territory Disaster Scraper',
...
app.get('/scrape', async (req, res) => {
  res.json({ message: 'NT scraping started' });
  scrapeNT().catch(console.error);
});
...
async function scrapeNT() {
  console.log(`\n=
 Scraping NT disasters at ${new Date().toISOString()}`);
  ...
  try {
    await page.goto('https://www.disasterassist.gov.au/find-a-disaster/australian-disasters?state=nt', {
  ...
  console.log(` NT scraping complete: ${allDisasters.length} disasters, ${activeCount} active, ${uniqueLGAs.length} LGAs affected`);
}

// Schedule hourly scraping
cron.schedule('0 * * * *', () => {
  console.log('ð Running scheduled NT scrape...');
  scrapeNT().catch(console.error);
});

// Start server
app.listen(PORT, () => {
  console.log(` NT Scraper running on port ${PORT}`);
  ...
  setTimeout(() => {
    console.log('=€ Running initial NT scrape...');
    scrapeNT().catch(console.error);
  }, 10000);
});