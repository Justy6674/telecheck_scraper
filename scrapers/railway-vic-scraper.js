...
/**
 * VICTORIA DISASTER SCRAPER FOR RAILWAY DEPLOYMENT
 * Scrapes VIC disasters and updates database
 * Expected: 7 pages, ~140 disasters
 */
...
console.log('🏛️ VIC Disaster Scraper Service Starting...');
...
app.get('/', (req, res) => {
  res.json({ 
    status: 'running',
    state: 'VIC',
    service: 'Victoria Disaster Scraper',
...
app.get('/scrape', async (req, res) => {
  res.json({ message: 'VIC scraping started' });
  scrapeVIC().catch(console.error);
});
...
async function scrapeVIC() {
  console.log(`\n🏛️ Scraping VIC disasters at ${new Date().toISOString()}`);
  ...
  try {
    await page.goto('https://www.disasterassist.gov.au/find-a-disaster/australian-disasters?state=vic', {
  ...
  console.log(`✅ VIC scraping complete: ${allDisasters.length} disasters, ${activeCount} active, ${uniqueLGAs.length} LGAs affected`);
}

// Schedule hourly scraping
cron.schedule('0 2 * * 0', () => {
  console.log('⏰ Running scheduled VIC scrape...');
  scrapeVIC().catch(console.error);
});

// Start server
app.listen(PORT, () => {
  console.log(`✅ VIC Scraper running on port ${PORT}`);
  ...
  setTimeout(() => {
    console.log('🚀 Running initial VIC scrape...');
    scrapeVIC().catch(console.error);
  }, 10000);
});