...
/**
 * NORTHERN TERRITORY DISASTER SCRAPER FOR RAILWAY DEPLOYMETAS
 * Scrapes TAS disasters and updates database
...
console.log('=
 TAS Disaster Scraper Service Starting...');
...
app.get('/', (req, res) => {
  res.json({ 
    status: 'running',
    state: 'TAS',
    service: 'Tasmania Disaster Scraper',
...
app.get('/scrape', async (req, res) => {
  res.json({ message: 'TAS scraping started' });
  scrapeTAS().catch(console.error);
});
...
async function scrapeTAS() {
  console.log(`\n=
 Scraping TAS disasters at ${new Date().toISOString()}`);
  ...
  try {
    await page.goto('https://www.disasterassist.gov.au/find-a-disaster/australian-disasters?state=tas', {
  ...
  console.log(` TAS scraping complete: ${allDisasters.length} disasters, ${activeCount} active, ${uniqueLGAs.length} LGAs affected`);
}
