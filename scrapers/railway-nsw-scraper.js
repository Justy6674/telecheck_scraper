...
/**
 * NSW DISASTER SCRAPER FOR RAILWAY DEPLOYMENT
 * Scrapes NSW disasters and updates database
 * Expected: 15 pages, ~300 disasters
 */
...
console.log('🏛️ NSW Disaster Scraper Service Starting...');
...
app.get('/scrape', async (req, res) => {
  res.json({ message: 'NSW scraping started' });
  scrapeNSW().catch(console.error);
});
...
async function scrapeNSW() {
  console.log(`\n🏛️ Scraping NSW disasters at ${new Date().toISOString()}`);
  ...
  try {
    await page.goto('https://www.disasterassist.gov.au/find-a-disaster/australian-disasters?state=nsw', {
  ...
  console.log(`✅ NSW scraping complete: ${allDisasters.length} disasters, ${activeCount} active, ${uniqueLGAs.length} LGAs affected`);
}

// Schedule hourly scraping
cron.schedule('0 2 * * 0', () => {
  console.log('⏰ Running scheduled NSW scrape...');
  scrapeNSW().catch(console.error);
});

// Start server
app.listen(PORT, () => {
  console.log(`✅ NSW Scraper running on port ${PORT}`);
  ...
  setTimeout(() => {
    console.log('🚀 Running initial NSW scrape...');
    scrapeNSW().catch(console.error);
  }, 10000);
});