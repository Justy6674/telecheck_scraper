...
/**
 * NORTHERN TERRITORY DISASTER SCRAPER FOR RAILWAY DEPLOYMEWA
 * Scrapes WA disasters and updates database
...
console.log('=
 WA Disaster Scraper Service Starting...');
...
app.get('/', (req, res) => {
  res.json({ 
    status: 'running',
    state: 'WA',
    service: 'Western Australia Disaster Scraper',
...
app.get('/scrape', async (req, res) => {
  res.json({ message: 'WA scraping started' });
  scrapeWA().catch(console.error);
});
...
async function scrapeWA() {
  console.log(`\n=
 Scraping WA disasters at ${new Date().toISOString()}`);
  ...
  try {
    await page.goto('https://www.disasterassist.gov.au/find-a-disaster/australian-disasters?state=wa', {
  ...
  console.log(` WA scraping complete: ${allDisasters.length} disasters, ${activeCount} active, ${uniqueLGAs.length} LGAs affected`);
}
