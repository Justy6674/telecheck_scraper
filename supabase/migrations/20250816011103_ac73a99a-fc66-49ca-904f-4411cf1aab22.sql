...
CREATE TABLE practice_registration (
  ...
  provider_types provider_type_enum[] NOT NULL DEFAULT ARRAY['GP'],
  contact_phone TEXT,
  address TEXT,
  state_code CHAR(3),
  postcode VARCHAR(10),
...
-- Performance indexes
CREATE INDEX CONCURRENTLY idx_disaster_lga_status_date 
ON disaster_declarations (lga_code, declaration_status, declaration_date DESC);

CREATE INDEX CONCURRENTLY idx_disaster_active_only 
ON disaster_declarations (declaration_date DESC) 
WHERE declaration_status = 'active';

CREATE INDEX CONCURRENTLY idx_disaster_postcodes 
ON disaster_declarations USING GIN(postcodes);

CREATE INDEX CONCURRENTLY idx_practice_user_id 
ON practice_registration (user_id);

CREATE INDEX CONCURRENTLY idx_verification_practice_date 
